...
-- set normal
function body:setNormalMapOffset(ox, oy)
  if self.normal then
    local offsetXStart = ox / self.normal:getWidth()
    local offsetX = offsetXStart + (self.normalWidth / self.normal:getWidth())
    local offsetYStart = oy / self.normal:getHeight()
    local offsetY = offsetYStart + (self.normalHeight / self.normal:getHeight())

    self.normalVert = {
      {0.0, 0.0,                            offsetXStart, offsetYStart},
      {self.normalWidth, 0.0,               offsetX, offsetYStart},
      {self.normalWidth, self.normalHeight, offsetX, offsetY},
      {0.0, self.normalHeight,              offsetXStart, offsetY}
    }
    self.normalMesh = love.graphics.newMesh(self.normalVert, "fan")
    self.normalMesh:setTexture(self.normal)
  end
